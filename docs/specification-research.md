...
| NVRAM/timekeeper | The MK48T08B is memory-mapped; the player stores game results, shell settings, FTS data, and CSD in NVRAM; maximum usable space is 31 KiB | `svcmanuals/cdi205.pdf`, PDF pp. 20 and 96 | The Mono-I model exposes a 32 KiB MK48T08-style device with eight clock registers and persists it by board. This agrees with the service evidence. | Verify the shell's free-space report and filesystem reservation rather than changing physical size. |
...
| Player control keys | `/pck` Play/Stop/Pause/Next/Previous/Search keys are an optional extension, separate from the base two-button pointer | `notes/technote073.2.pdf`, printed pp. 1-3 | A frontend Start overlay should remain a host control and must not silently become a third CD-i pointer button. | If `/pck` is emulated later, expose it as a distinct optional peripheral. |
