...
| MPEG transitions | Pause/continue, rapid abort/restart, starting on EOS+SOS, sequence changes, and PCL flushing have documented edge cases | `notes/technote088.pdf`, printed pp. 1-6 and issue summary | Relevant to The 7th Guest transitions, but it did not prove the historical rare B-picture root cause. Two exact current five-play runs are error-free after the shared-IN4 correction, so that older failure is superseded under changed transport prerequisites. | Add one isolated synthetic test per transition rule before reworking live-title behavior. |
