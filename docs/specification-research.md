...
| CDIC disc audio | On Mono-I, the first selected XA sector enters `$2800` and reports DBUF low nibble 4; the second uses `$3200`/5. CDDA and CD-fed XA reading do not start audible playback: software writes `AUDCTL $0800` after the first buffer/IRQ. AUDCTL bit 0 is set by an `$ff`-coded stop and cleared by its status read, not toggled on every read. | Independent Mono-I hardware captures and tests in `Slamy/CDIC_BlackBoxAnalyzer` revision `e861f76`, `doc/cdic_manual.md`, `src/test_xa_play.c`, `src/test_cdda_play.c`, and `src/test_audiomap.c` | Implemented as separate receipt and playback state. The MAME-derived direct `play_realtime_audio_sector` shortcut is removed; focused tests cover first/second buffer placement, no PCM before `$0800`, CDDA gating, and the one-shot stop latch. Bounded Earth Command and Alien Gate runs retain byte-identical WAV output versus `c012d17`. | Manually recheck CDDA, XA start/stop, Earth Command termination, and Alien Gate firing. Model SLAVE `$82/$83` plus AD7528 output gating separately; these CDIC traces do not establish automatic physical-mixer unmute. |
...
| MPEG transitions | Pause/continue, rapid abort/restart, starting on EOS+SOS, sequence changes, and PCL flushing have documented edge cases | `notes/technote088.pdf`, printed pp. 1-6 and issue summary; Philips FMVDemo `SUN/SRC/APPL/play_control.c` and `multilingual.c` | Synthetic transition rules and the deterministic five-play run pass. The native FMVDemo pause/continue scenario now isolates an unresolved CDIC/CDFM resume stall after `mv_continue` + `ss_cont`; its multilingual scenario isolates one MP2 boundary error after `ma_cntrl` changes the FMA stream in place. | Resolve the `$24` CDIC resume contract from driver/firmware evidence, then determine the correct Layer-II FIFO boundary for in-place stream selection. Retain both local scripts as executable gates. |
| CDIC sound maps | `SM_Done` means the last sector reached the audio processor buffer, not that it became inaudible; buffered audio may continue | `notes/technote079.pdf`, printed pp. 1-2 | Per-half refill interrupts align with transfer semantics. Audible stop/replace latency needs explicit coverage. | Test one- and two-sector sound maps, done signaling, replacement, and the remaining audible tail. |
